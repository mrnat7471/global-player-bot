...
class reachCog(commands.Cog):
    ...
    @bot.command(name='thequeue')
    async def thequeue(self, ctx):
        number = len(queue[f"{ctx.guild.id}"])
        embed = discord.Embed(colour=discord.Colour.blue(), description=f"You have {number} in the queue. To clear please disconnect and reconnect the bot.")
    ...
    async def queue(self, ctx, *, message):
        presenterdict = {}
        ...
        if cachedEmoji == "1️⃣":
            ...
            showid = None
        amount = len(queue[f"{ctx.guild.id}"])

        amount = amount + 1
        
        queue[f"{ctx.guild.id}"].update({amount: showid})

        embed = discord.Embed(colour=discord.Colour.blue(), description="Added to the queue.")
        embed.set_author(name='Global Player', icon_url=self.bot.user.avatar_url)
        embed.timestamp = datetime.datetime.utcnow()
        embed.set_footer(text=f'Global Player Bot', icon_url=self.bot.user.avatar_url)
        message = await ctx.channel.send(embed=embed)
    ...
    async def catchup(self, ctx, *, message):
        ...
        def repeat(guild, voice, audio_number):
            audio_number = audio_number + 1
            songqueue = queue[f"{ctx.guild.id}"]
            showid = songqueue[audio_number]
            VoiceClient.play(discord.FFmpegPCMAudio(showid), after=lambda e: repeat(ctx.guild, VoiceClient, audio_number))
        ...
        #showid = "https://api.reachradio.co.uk/prime.wav"
        queue[f"{ctx.guild.id}"] = {}
        audio_number = 0
        VoiceClient.play(discord.FFmpegPCMAudio(showid), after=lambda e: repeat(ctx.guild, VoiceClient, audio_number))
